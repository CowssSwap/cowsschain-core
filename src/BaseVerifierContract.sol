...
abstract contract BaseVerifierContract is EIP712 {
    ...
    function verifySignature(bytes memory _json, bytes memory _signature) external view returns (address) {
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(keccak256(abi.encode(fullOrderType)), keccak256(_json))));
